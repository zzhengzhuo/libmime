...
class Libmime {
     ...
     */
    decodeWord(charset, encoding, str, subjectSep, isSepBase64, isSubject) {
        ...
        return libcharset.decode(str, charset) + (subjectSep || '');
    }
        ...
        return (
            (str || '')
                ...
                .replace(/=\?([\w_\-*]+)\?([QqBb])\?([^?]*)\?=/g, (m, charset, encoding, text) =>
                    this.decodeWord(charset, encoding, text, subjectSep || '', isSepBase64, true)
                )
        );
    }
