...
class Libmime {
     ...
     */
    decodeWord(charset, encoding, str, subjectSep, isSepBase64, isSubject, encodings) {
        // RFC2231 added language tag to the encoding
        // see: https://tools.ietf.org/html/rfc2231#section-5
        // this implementation silently ignores this tag
        isSepBase64 = isSepBase64 || false;
        ...
        const ret = libcharset.decode(str, charset);
        if (isSubject) {
            ret.split(subjectSep).forEach(_s => {
                this.isSubBase64.push(encodings === 'B');
            });
        }

        return ret + (subjectSep || '');
    }
    ...
    decodeSubject(str, subjectSep, isSepBase64) {
        let encodings = [];
        return (
            (str || '')
                .toString()
                ...
                // join base64 encoded words
                .replace(/(\?=)?__\x00JOIN\x00__(=\?([^?]+)\?[QqBb]\?)?/g, subjectSep || '')
                // remove spaces between mime encoded words
                .replace(/(=\?[^?]+\?[QqBb]\?[^?]*\?=)\s+(?==\?[^?]+\?[QqBb]\?[^?]*\?=)/g, '$1')
                // decode words
                .replace(/=\?([\w_\-*]+)\?([QqBb])\?([^?]*)\?=/g, (m, charset, encoding, text) =>
                    this.decodeWord(charset, encoding, text, subjectSep || '', isSepBase64, true, encodings.shift())
                )
        );
    }
