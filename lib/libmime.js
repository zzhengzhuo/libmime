...
class Libmime {
     ...
     */
    decodeWord(charset, encoding, str, isSepBase64) {
    ...
    decodeSubject(str) {
        ...
        while (item = re.exec(str)) {
            arr.push([item.index, item[0].length, item[1], item[2], item[3]]);
        }

        if (arr.length == 0) {
            this.isSubBase64.push(false);
            return [str];
        }
        ...
        for (let elm of arr) {
            if (elm[0] > preIndex) {
                ret.push(str.substring(preIndex, elm[0]));
                this.isSubBase64.push(false);
            }
            ret.push(this.decodeWord(elm[2], elm[3], elm[4], true));
            if (elm[3] == 'b' || elm[3] == 'B') {
                this.isSubBase64.push(true);
            } else {
                this.isSubBase64.push(false);
            }
            preIndex = elm[0] + elm[1];
        }
        if (preIndex < str.length) {
            ret.push(str.substring(preIndex, str.length));
            this.isSubBase64.push(false);
        }
